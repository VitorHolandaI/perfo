...
pub fn page(page: usize, lang: Lang, theme: &Theme) -> (String, Vec<Line<'static>>) {
    let (title, rows): (String, &[HelpRow<'static>]) = match (lang, page) {
        (Lang::Pt, 0) => (
            ...
            &[
                hdr("PAINEIS — menu"),
                key("m", "abre o menu para escolher CPU+PROCS, IO ou NET"),
                key("1-3", "focam diretamente CPU+PROCS, IO ou NET"),
                key("Tab", "na CPU alterna foco entre CORES e PROCESSOS"),
                key("←↑↓→", "com foco CORES, move o nucleo selecionado"),
                key("Enter", "filtra processos pelo nucleo selecionado (Esc limpa)"),
                key("↑ ↓", "com foco PROCESSOS, seleciona processo"),
                txt(""),
                hdr("ACOES"),
        ...
        (Lang::En, 0) => (
            ...
            &[
                hdr("PANELS — menu"),
                key("m", "open menu to choose CPU+PROCS, IO, or NET"),
                key("1-3", "focus CPU+PROCS, IO, or NET directly"),
                key("Tab", "in CPU, switch focus between CORES and PROCESSES"),
                key("←↑↓→", "with CORES focused, move the selected core"),
                key("Enter", "filter processes by selected core (Esc clears)"),
